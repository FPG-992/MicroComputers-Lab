...
int main(void){
    ...
    char passcode[3] = {'1', '7', '\0'};
    char entered_passcode[3] = {'\0', '\0', '\0'};
    ...
    char key = keypad_to_ascii();
        if(key != '\0') { // ??????? ?? ??????? ??? ???????? ???????
            lcd_clear();   
            lcd_data(key);  // ????????? ??? ????????? ???? LCD
            times_pressed++; //increase counter
            entered_passcode[times_pressed - 1] = key; //store the key in the entered passcode

            if (times_pressed==2 && entered_passcode[0] == passcode[0] && entered_passcode[1] == passcode[1]){
                    lcd_clear();
                    lcd_data('O');
                    lcd_data('K');
                    //PB0 to PB5 turn on and off for 5 seconds each, 500ms on 500ms off
                    PORTB = 0b00111111;
                    _delay_ms(5000);
                    PORTB = 0b00000000;
                    times_pressed = 0; 
                    entered_passcode[0] = '\0';
                    entered_passcode[1] = '\0';
                    } else if (times_pressed==2 && entered_passcode[0] != passcode[0] || entered_passcode[1] != passcode[1]){
                    lcd_clear();
                    lcd_data('N');
                    lcd_data('O');
                    for (int i = 0; i < 3; i++){
