...
unsigned char twi_readAck() {
    // Read one byte from TWI device, requesting more
...
void display_number(uint8_t input) {
    // Display a single digit number on LCD
    lcd_data(0b00110000 | (input & 0x0F));  // Convert number to ASCII
}
...
uint8_t one_wire_reset() {
    ...
    return (temp & (1 << 4)) ? 0 : 1;  // Return 1 if device is present
}
...
void display_pressure(double input) {
    ...
    binary_to_bcd(decimal);
    display_number(thousands);
    // You can display more decimal digits if needed
}
...
int main(void) {
    ...
    while (1) {
        ...
        _delay_ms(10000);  // Wait to allow reading the message
    }
}
